...
class FCN1s(nn.Module):
    ...
    def forward(self, x):
        ...
        score = self.bn3(score + x2)                         # element-wise add, size=[n, 128, x.h/4, x.w/4]
        score = self.relu(self.deconv4(score))               # size=[n, 64, x.h/2, x.w/2]
        score = self.bn4(score + x1)                         # element-wise add, size=[n, 64, x.h/2, x.w/2]
        score = self.bn5(self.relu(self.deconv5(score)))     # size=[n, 32, x.h, x.w]
        score = self.classifier(score)                       # size=[n, n_class, x.h, x.w]
        return score
...
# 参数model='vgg16'  因为 VGG模型有VGG11, VGG13, VGG16以及VGG19等类型，这里使用model缺省参数VGG16。
# 通过cfg[model]就可以获得对应VGG模型配置文件，然后通过make_layers()就把VGG模型建立起来。
class VGGNet(VGG):
    def __init__(self, pretrained=True, model='vgg16', requires_grad=True, remove_fc=True, show_params=False):
        ...
        if pretrained:
            ...
            # vgg16.load_state_dict(torch.load('/home/wp/.torch/models/vgg16-397923af.pth'))
