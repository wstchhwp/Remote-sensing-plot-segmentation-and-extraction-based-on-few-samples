...
class CAttUNet(object):
    #--------------------------------------------------------------#
    #   注意修改model_path、num_classes和backbone,使其符合自己的模型
    #--------------------------------------------------------------#
    _defaults = {
        "model_path"        :   '图像分割/FCN/logs_cattunet/Epoch97-Train_Loss0.5118-Val_Loss0.5055-Train_Dice0.4880-Val_Dice0.4796.pth',
    ...
    def generate(self):  
        self.net = CAttU_Net(num_classes=2)
        # 加载训练好的模型
    ...
    def detect_image(self, image):
        ...
        #print(np.unique(pr))
        cv2.imwrite("图像分割/FCN/提交结果/CAttU-Net_2996.png",np.uint8(pr),[cv2.IMWRITE_PNG_COMPRESSION, 0])
